...
export default function Hero() {
  return (
    <section
      aria-label="Hero"
      className="bg-[#1C1D20] h-screen flex flex-col justify-between relative"
      style={{ padding: "clamp(60px, 10vh, 120px) clamp(20px, 4vw, 80px) clamp(40px, 5vh, 80px)" }}
    >
      <div className="flex md:flex-row flex-col justify-between items-center text-white">
        ...
        <Text animateOnScroll={false} delay={1}>
          <p className="label text-center text-white/70 md:max-w-[20ch] mt-4 md:mt-0 leading-[1.6]">
            Hi, I&apos;m a software developer from India — I craft
            production-ready web & mobile apps
          </p>
        </Text>
      </div>
      ...
      <div className="flex md:flex-row flex-col items-center justify-between text-white z-1">
        <Text animateOnScroll={false} delay={1}>
          <p className="label flex flex-col justify-center items-center gap-y-4 mt-10 text-white/70">
            <span className="text-center">
              I identify and eliminate bugs for a living
            </span>
            <span>
              <svg
                width="19"
